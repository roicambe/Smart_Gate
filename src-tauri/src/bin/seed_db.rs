...
fn main() -> Result<()> {
    ...
    let dept_id: i64 = conn.query_row(
    ...
    if person_id > 0 {
        ...
        println!("Successfully inserted student: Roi Yvann Montemayor Cambe, 23-00193, BSIT, 3rd Year");
    } else {
    ...
    conn.execute(
        ...
        params!["Main Gate - Entrance", "entrance", "Main Gate - Exit", "exit"],
    )?;
