...
import { Users, Search, Edit2, Trash2, UserPlus, Eye, Check, AlertTriangle, ChevronLeft, ChevronRight, Mail, Filter, Upload, FileSpreadsheet, Download, GraduationCap, BookOpen, Loader2, Building, UserCircle2 } from 'lucide-react';
...
const buildUserPayload = (mainRole, subRoles, data) => {
    const roles = [mainRole, ...subRoles];
    const isEmployee = mainRole === 'employee';
    const isStudent = mainRole === 'student';
    const isVisitor = mainRole === 'visitor';
...
export const UserManagement = ({ adminSession, branding }) => {
    ...
    useEffect(() => {
        ...
        if (isEmailAutoGenerated && isAcademic) {
    ...
    useEffect(() => {
        ...
        const isVisitor = formData.mainRole === 'visitor';
    ...
    }, [formData.email, formData.mainRole, branding?.strict_email_domain]);

    const activeIdRule = ID_NUMBER_RULES[formData.mainRole];
    ...
    const handleRegisterSubmit = async (e) => {
        ...
        try {
            const payload = buildUserPayload(formData.mainRole, formData.subRoles, formData);
            await invoke('register_user', { ...payload, isActive: formData.is_active, activeAdminId: adminSession?.account_id });

            if (formData.mainRole === 'visitor' && formData.email) {
                    ...
                    <form onSubmit={showEditModal ? handleEditSubmit : handleRegisterSubmit} className="space-y-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    {[
                                        { id: 'student', label: 'Student', color: 'text-blue-400', bg: 'bg-blue-400/10', border: 'border-blue-500/30' },
                                        { id: 'employee', label: 'Employee', color: 'text-amber-400', bg: 'bg-amber-400/10', border: 'border-amber-500/30' },
                                        { id: 'visitor', label: 'Visitor', color: 'text-emerald-400', bg: 'bg-emerald-400/10', border: 'border-emerald-500/30' }
                                    ].map((r) => {
                                        const isSelected = formData.mainRole === r.id;
                                        return (
                                            <label key={r.id} className={`relative flex flex-col items-center justify-center gap-2 p-4 rounded-xl border cursor-pointer transition-all duration-300 ${isSelected ? `${r.bg} ${r.border} ring-1 ring-white/20` : 'bg-white/[0.03] border-white/5 text-white/40 hover:bg-white/[0.06] hover:border-white/10'}`}>
                                                <input type="radio" name="mainRole" value={r.id} checked={isSelected} onChange={(e) => setFormData({ ...formData, mainRole: e.target.value, subRoles: [] })} className="sr-only" />
                                                <span className={`text-sm font-bold tracking-wide ${isSelected ? 'text-white' : ''}`}>{r.label}</span>
                                                {isSelected && <div className={`absolute -top-1 -right-1 p-1 rounded-full ${r.bg} border ${r.border} shadow-lg`}><Check className="w-3 h-3 text-white" /></div>}
                                            </label>
                                        );
                                    })}
                                </div>
                            ...
                            {formData.mainRole === 'employee' && (
                                <div className="p-4 bg-white/5 border border-white/10 rounded-xl space-y-4 animate-in fade-in slide-in-from-top-2">
                                    <div className="flex items-center justify-between">
                                        <label className="block text-xs text-white/60 font-semibold uppercase tracking-wider">Specific Designation</label>
                                        <span className="text-[10px] bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded-full border border-amber-500/30 font-bold uppercase">Sub-Roles</span>
                                    </div>
                                    <div className="flex flex-wrap gap-2.5">
                                        {(() => {
                                            const employeeRole = availableRoles.find(r => r.role_name.toLowerCase() === 'employee');
                                            const employeeId = employeeRole ? employeeRole.role_id : 2;
                                            return availableRoles.filter(r => !r.is_main_role && r.parent_role_id === employeeId).map(role => {
                                                const isActive = formData.subRoles.includes(role.role_name);
                                                return (
                                                    <label key={role.role_id} className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl border cursor-pointer transition-all duration-200 ${isActive ? 'bg-amber-500/20 border-amber-500/50 text-amber-200 shadow-[0_0_15px_rgba(245,158,11,0.1)]' : 'bg-black/40 border-white/10 text-white/50 hover:border-white/30'}`}>
                                                        <input 
                                                            type="checkbox" 
                                                            className="hidden" 
                                                            checked={isActive}
                                                        ...
                                                        <div className={`w-4 h-4 rounded flex items-center justify-center transition-all ${isActive ? 'bg-amber-500 scale-110' : 'bg-white/10'}`}>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        <label className="block text-xs text-white/60 mb-1 font-medium">
                                            {formData.mainRole === 'student' ? 'Student ID Number' : formData.mainRole === 'visitor' ? 'Visitor ID Number' : 'Employee ID Number'} <span className="text-rose-500 text-base font-bold ml-0.5">*</span>
                                        </label>
                                        <input
                                            required
                                            type="text"
                                            inputMode={formData.mainRole === 'visitor' ? 'text' : 'numeric'}
                                            value={formData.id_number}
                                            maxLength={activeIdRule.maxLength}
                                            onChange={e => setFormData({
                                                ...formData,
                                                id_number: formData.mainRole === 'visitor' ? e.target.value.toUpperCase() : e.target.value,
                                            })}
                                    ...
                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                        <div>
                                            <label className="mb-1 flex min-h-[24px] items-end text-xs font-medium text-white/60">Email Address {formData.mainRole === 'visitor' && <span className="ml-0.5 text-base font-bold text-rose-500">*</span>}</label>
                                            <input 
                                                ref={emailRef}
                                                required={formData.mainRole === 'visitor' || (branding?.strict_email_domain && formData.mainRole !== 'visitor')} 
                                                type="email" 
                                ...
                                {formData.mainRole === 'student' && (
                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 animate-in fade-in">
                                        <div>
                                            ...
                                            <select required={formData.mainRole === 'student'} value={formData.program_id} onChange={e => setFormData({ ...formData, program_id: e.target.value })} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-white/20 focus:outline-none appearance-none">
                                        ...
                                        <div>
                                            ...
                                            <select required={formData.mainRole === 'student'} value={formData.year_level} onChange={e => setFormData({ ...formData, year_level: e.target.value })} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-white/20 focus:outline-none appearance-none">
                                ...
                                {formData.mainRole === 'visitor' && (
                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 animate-in fade-in">
                                        <div>
                                            ...
                                            <input required={formData.mainRole === 'visitor'} type="text" value={formData.purpose_of_visit} onChange={e => setFormData({ ...formData, purpose_of_visit: e.target.value })} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-white/20 focus:outline-none" />
                                        </div>
                                        <div>
                                            ...
                                            <input required={formData.mainRole === 'visitor'} type="text" value={formData.person_to_visit} onChange={e => setFormData({ ...formData, person_to_visit: e.target.value })} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-white/20 focus:outline-none" />
                                        </div>
                                    </div>
                                )}
