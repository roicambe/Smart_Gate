...
export const AdminLayout = ({ view, setView, setIsAdminLoggedIn, adminSession, branding, fetchBranding }) => {
    ...
    const rawNavItems = [
        ...
        { id: 'admin_organizational', label: 'Organizational Structure', icon: Building, reqSuper: true },
    ...
    const renderView = () => {
        switch (view) {
            ...
            case 'admin_organizational': return <OrganizationalStructure branding={branding} adminSession={adminSession} />;
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-4 w-full px-4 py-3 rounded-lg transition-all focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-left ${isActive
                            ...
                            >
                                <Icon className={`w-5 h-5 shrink-0 text-white ${isActive ? 'text-blue-400 opacity-100' : 'opacity-80'}`} />
